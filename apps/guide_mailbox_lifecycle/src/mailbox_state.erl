...
%%%
%%% One mailbox per citizen, keyed by `citizen_did' (the stream id). Tracks
%%% the mailbox's own lifecycle status plus every letter it holds. Letters
%%% are never deleted from state (event-sourced history is the point) --
%%% `archived' marks a letter as no longer active without erasing it.
%%% @end
-module(mailbox_state).
-behaviour(evoq_state).

-export([new/1, apply_event/2, to_map/1]).
-export([status/1, citizen_did/1, get_letter/2, is_open/1]).
...
-record(state, {
    ...
    status = unopened :: unopened | open | closed,
...
-spec status(state()) -> unopened | open | closed.
status(#state{status = S}) -> S.

-spec is_open(state()) -> boolean().
is_open(#state{status = open}) -> true;
is_open(_) -> false.
...
apply_event(S, #{event_type := <<"mailbox_opened_v1">>} = Ev) ->
    S#state{citizen_did = maps:get(citizen_did, Ev), status = open};
