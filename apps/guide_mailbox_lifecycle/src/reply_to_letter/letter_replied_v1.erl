...
%%% sides of the reply agree on the new letter's identity.
-module(letter_replied_v1).
-behaviour(evoq_event).

-export([event_type/0]).
-export([new/1, from_map/1, to_map/1]).
-export([get_letter_id/1, get_original_sender_did/1, get_reply_letter_id/1,
...
-spec new(map()) -> t().
new(#{letter_id := Id, original_sender_did := To} = Params) ->
    #letter_replied_v1{
        letter_id = Id,
...
from_map(#{letter_id := Id, original_sender_did := To, reply_letter_id := RId,
           replied_at := At} = M)
  when is_binary(Id), is_binary(To), is_binary(RId), is_integer(At) ->
