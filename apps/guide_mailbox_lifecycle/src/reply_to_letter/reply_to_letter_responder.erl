%%% @doc RESPONDER for the `hecate_mail.reply_to_letter` mesh capability.
%%%
%%% Gated behind `mailbox_ownership_proof': replying acts on the
%%% replier's OWN mailbox (marks the original letter answered there,
%%% then delivers the reply into the original sender's mailbox), so the
%%% caller must prove they hold the private key for `citizen_did'
%%% before this dispatches anything -- see that module's doc for why.
%%% @end
...
-define(PROCEDURE, <<"hecate_mail.reply_to_letter">>).
...
proven_reply(ok, Payload) ->
    ...
    case reply_to_letter_v1:new(Params) of
